...
async fn test_watchdog_rule_activation() {
    ...
    // 测试符合条件的事件
    let triggered: Vec<String> = watchdog_agent
        .process_event(&ToolExecutionEvent::PostExecute {
...
async fn test_watchdog_rule_status_matching() {
    ...
    let rule = WatchdogRule::new(
        "status_match_rule",
        "status_test_tool",
        TriggerCondition::StatusMatches {
            expected_status: "failed".to_string(),
        },
        "failure_handler",
    );
